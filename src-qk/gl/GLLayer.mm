...
Utf8 frameBufferStatusDesc(GLenum status) {
  switch (status) {
      CASE_RET_TOK_SPLIT(GL_FRAMEBUFFER_, INCOMPLETE_ATTACHMENT);
      CASE_RET_TOK_SPLIT(GL_FRAMEBUFFER_, INCOMPLETE_MISSING_ATTACHMENT);
      CASE_RET_TOK_SPLIT(GL_FRAMEBUFFER_, INCOMPLETE_DIMENSIONS);
      CASE_RET_TOK_SPLIT(GL_FRAMEBUFFER_, UNSUPPORTED);
    default:
      return "unknown";
  }
}
...
void describePFA(CGLPixelFormatObj format, GLint virtualScreen, CGLPixelFormatAttribute attribute, const char *name) {
  GLint val;
...
- (CGLPixelFormatObj)copyCGLPixelFormatForDisplayMask:(uint32_t)mask {
  ...
  CGLError e = CGLChoosePixelFormat((const CGLPixelFormatAttribute*)attributes.bytes, &pixelFormat, &virtualScreenCount);

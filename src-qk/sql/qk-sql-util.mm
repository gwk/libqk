...
NSString* sql_failure_str(sqlite3* db, int code) {
  return fmt(@"SQLite failure: %@; msg: '%s'", sql_code_description(code), sqlite3_errmsg(db));
}


const char* sql_code_string(int code) {
  switch (code) {
      CASE_RET_TOK(SQLITE_OK);
      CASE_RET_TOK(SQLITE_ERROR);
      CASE_RET_TOK(SQLITE_INTERNAL);
      CASE_RET_TOK(SQLITE_PERM);
      CASE_RET_TOK(SQLITE_ABORT);
      CASE_RET_TOK(SQLITE_BUSY);
      CASE_RET_TOK(SQLITE_LOCKED);
      CASE_RET_TOK(SQLITE_NOMEM);
      CASE_RET_TOK(SQLITE_READONLY);
      CASE_RET_TOK(SQLITE_INTERRUPT);
      CASE_RET_TOK(SQLITE_IOERR);
      CASE_RET_TOK(SQLITE_CORRUPT);
      CASE_RET_TOK(SQLITE_NOTFOUND);
      CASE_RET_TOK(SQLITE_FULL);
      CASE_RET_TOK(SQLITE_CANTOPEN);
      CASE_RET_TOK(SQLITE_PROTOCOL);
      CASE_RET_TOK(SQLITE_EMPTY);
      CASE_RET_TOK(SQLITE_SCHEMA);
      CASE_RET_TOK(SQLITE_TOOBIG);
      CASE_RET_TOK(SQLITE_CONSTRAINT);
      CASE_RET_TOK(SQLITE_MISMATCH);
      CASE_RET_TOK(SQLITE_MISUSE);
      CASE_RET_TOK(SQLITE_NOLFS);
      CASE_RET_TOK(SQLITE_AUTH);
      CASE_RET_TOK(SQLITE_FORMAT);
      CASE_RET_TOK(SQLITE_RANGE);
      CASE_RET_TOK(SQLITE_NOTADB);
      CASE_RET_TOK(SQLITE_ROW);
      CASE_RET_TOK(SQLITE_DONE);

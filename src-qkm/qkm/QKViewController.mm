...
+ (BOOL)autoLayoutContentView {
  return YES;
}


- (void)viewDidLayoutSubviews {
  // note: ios 7.0 has weird behavior where if you call insetFrame scrolling breaks.
  // since UIViewController handles scroll view insets automatically anyway, just guard here.
  if (![self.view isKindOfClass:[UIScrollView class]] && [self.class autoLayoutContentView]) {
    self.contentView.frame = self.insetFrame;
  }
}
